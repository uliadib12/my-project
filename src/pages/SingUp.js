...
function Singup(props) {  
    ...
    const register = (e) => {
        ...
        if(pass !== confrmpass){
            setisLoading(false)
            setisError({condition: true, massage:"Password anda Tidak Sama"})
            return 0
        }
        else{
            ...
            <div className="flex justify-center py-10 flex-grow bg-gradient-to-r from-blue-400 to-blue-800 max-h-screen overflow-y-hidden h-screen">
                <div className="box-border bg-blue-50 w-96 rounded-3xl py-11 px-12 relative overflow-hidden shadow-2xl">
                    ...
                    <form ref={formRef}>
                        ...
                        <div className="flex h-20 items-center">{isLoading && <object type="image/svg+xml" width={"60px"} data={Loadingbar}>svg-animation</object>}{isError.condition && <div className="text-red-600 text-center"><div className="p-2 bg-red-300 border-2 border-red-500 rounded-lg">Error: {isError.massage}</div></div>}</div>
                        </div>
